...
This project demonstrates application scalability using Kubernetes and Horizontal Pod Autoscaling (HPA). A containerized social media application was deployed on a local Minikube cluster through a Kubernetes Deployment, exposed using a NodePort Service, and monitored using the Kubernetes Metrics Server. An HPA was configured to automatically scale the number of pods based on CPU utilization, and the scaling behavior was demonstrated under simulated load.
...
- Minikube
- kubectl
- Container image `registry.k8s.io/hpa-example` (or `social-media-app:latest`)
...
| Field | Value |
|---|---|
| Deployment name | `social-media-deployment` |
| Replicas | `2` |
| Label | `social-media-app` |
| Container name | `social-media-container` |
...
Since Minikube runs its own container runtime, if using a local Docker image, it can be loaded directly into the Minikube environment.
...
```cmd
minikube image ls | findstr social
```
...
| Field | Value |
|---|---|
| HPA name | `social-media-hpa` |
| Target | `social-media-deployment` |
| Minimum replicas | `2` |
| Maximum replicas | `5` |
| CPU target | `50%` |
...
Figure: HPA created and targeting the social media deployment.
...
The Deployment initially ran with 2 replicas and automatically scaled to 3 replicas (or more) as CPU utilization increased. The application remained accessible through the NodePort Service throughout the scaling event.
