...
| :---: | :--- | :--- | :--- | :--- |
| ✅ | `P2_SS_01` | `P2_01_multistage_docker_build_and_ps.png` | Terminal showing `docker build -t react-app-nginx:latest .` logs, `docker images` (~25MB image size), and `docker ps` confirming active container on port 8080. | Proves multi-stage compilation (`node:18-alpine` -> `nginx:alpine`), image optimization, and container runtime. |
...
| ✅ | `P2_SS_03` | `P2_03_nginx_access_logs.png` | Terminal output of `docker logs react-app-container` showing HTTP GET requests returning `200 OK` from Nginx server. | Proves Nginx web server handling client requests via reverse proxy. |
