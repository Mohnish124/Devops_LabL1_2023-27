...
| ✅ | `P6_SS_03` | `P6_03_service_and_application_verification.png` | Terminal execution of `kubectl get svc -n social-media` and HTTP test queries via `curlimages/curl` hitting `/health` and `/api/posts` REST endpoints with live pod hostname. | Functional validation of social media REST service exposure, HTTP 200 health probe responses, and serving pod identity tracking. |
